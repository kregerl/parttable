    ...
    if let Some(partition) = maybe_partition {
        ...
        let maybe_starting_lba: Option<usize> = match partition {
            PartitionTableEntry::Mbr(mbr_partition_table_entry) => {
                if mbr_partition_table_entry.partition_type() == NTFS_MBR_PARTITION_TYPE {
                    Some(mbr_partition_table_entry.starting_lba());
                }
                None
            }
            PartitionTableEntry::Gpt(gpt_partition_table_entry) => {
                if gpt_partition_table_entry.partition_type() == NTFS_GPT_PARTITION_TYPE {
                    Some(gpt_partition_table_entry.starting_lba() as usize);
                }
                None
            }
        };
